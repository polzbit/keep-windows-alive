from PyQt5.QtCore import Qt, QPoint, pyqtSlot, pyqtProperty, QPropertyAnimation, QEasingCurve, pyqtSignal
...
class SwitchCircle(QWidget):
	...
	def mouseReleaseEvent(self, event):
		try:
			go_to = min(self.move_range, key=lambda x: abs(x - self.new_x))
...
class SwitchWidget(QCheckBox):
	...
	def __init__(self, parent=None, checked: bool = False):
		...
		self.animation_curve = QEasingCurve.OutBounce
		self.animation_duration = 300
		self.__circle = SwitchCircle(self, (3, self.width() - 19), self.circle_color, self.animation_curve,
		                             self.animation_duration)
		self.active_color = "#4287f5"
		self.auto = False
		self.pos_on_press = None
		if checked:
			self.__circle.move(self.width() - 19, 3)
	...
	def start_animation(self, checked):
		...
		if checked:
			self.animation.setEndValue(QPoint(self.width() - 19, self.__circle.y()))
			self.setChecked(True)
		if not checked:
